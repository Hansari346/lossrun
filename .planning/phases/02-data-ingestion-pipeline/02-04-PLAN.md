...
must_haves:
  ...
  key_links:
    ...
    - from: "src/lib/parsing.ts"
      ...
      via: "import detectCompositeFields"
      pattern: "import.*detectCompositeFields.*from.*composite-fields"
    - from: "src/lib/parsing.ts"
...
<task type="auto">
  ...
  **New imports to add:**
  ...
  import { detectCompositeFields } from "./composite-fields";
  ...
  **Changes to `applyMappingAndLoad(mappingOverrides: Record<string, number>)`:**
  ...
  3. **Replace the `rawRows.forEach` parsing loop** with the validation engine:
     ```typescript
  ...
  4. **Update the status message** to include validation info:
     ```
  ...
  5. **Keep `populateSiteFilter` and `populateAdjustmentsFromData` calls** at the end — these are unchanged.
