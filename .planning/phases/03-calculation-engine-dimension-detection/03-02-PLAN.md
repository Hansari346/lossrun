...
<task type="auto">
  <name>Task 1: Reactive store signals and dimension panel component</name>
  <files>src/state/store.ts, src/components/dimension-panel.tsx</files>
  <action>
  ...
  **2. Create src/components/dimension-panel.tsx:**
  ...
  - resetState() clears dimensionOverrides and no longer tries to write to results
